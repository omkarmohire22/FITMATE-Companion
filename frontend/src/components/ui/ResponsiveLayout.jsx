        ...
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            {headers.map((header, idx) => (
              <th
                key={idx}
                className="px-4 md:px-6 py-3 text-left font-semibold text-gray-900"
              >
                {header}
              </th>
            ))}
          ...
          {rows.map((row, ridx) => (
            <tr key={ridx} className="hover:bg-gray-50 transition-colors">
              {row.map((cell, cidx) => (
                <td
                  key={cidx}
                  className="px-4 md:px-6 py-3 text-gray-700"
                >
                  {cell}
                </td>
              ))}
      ...
      {items.map((item, idx) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.subtitle && (
              <p className="text-small text-gray-600 mt-0.5">{item.subtitle}</p>
            )}
          </div>
    ...
    <div className={`dashboard-grid-4 ${className}`}>
      {stats.map((stat, idx) => (
        <div key={idx} className="dashboard-card p-4 md:p-6">
          <p className="text-small text-gray-600 font-medium">{stat.label}</p>
          <p className="heading-2 text-gray-900 mt-2">{stat.value}</p>
          {stat.change && (
            <p
