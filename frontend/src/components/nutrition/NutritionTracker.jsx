...
export default function NutritionTracker() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Navigation Tabs */}
      <div className={`flex gap-2 border-b rounded-t-2xl -mb-px shadow-sm ${isDark ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'}`}>
        <button
          onClick={() => setActiveTrackerTab('today')}
          className={`px-4 sm:px-6 py-3 font-semibold text-sm sm:text-base border-b-2 transition-colors ${activeTrackerTab === 'today'
        ...
        <button
          ...
          className={`px-4 sm:px-6 py-3 font-semibold text-sm sm:text-base border-b-2 transition-colors flex items-center gap-2 ${activeTrackerTab === 'analytics'
            ? 'border-green-500 text-green-600'
            : `border-transparent ${isDark ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-900'}`
            }`}
        >
      ...
      {activeTrackerTab === 'today' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Calories Card */}
            <div className={`rounded-2xl p-5 sm:p-6 border shadow-sm transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-orange-900/30 to-orange-800/20 border-orange-700/50' : 'bg-white border-orange-100 shadow-orange-100/50'}`}>
              <div className="flex items-center justify-between mb-3">
                <h3 className={`text-sm font-bold uppercase tracking-wide transition-colors duration-300 ${isDark ? 'text-orange-300' : 'text-orange-600'}`}>Calories</h3>
                <span className="text-2xl">🔥</span>
              </div>
              <div className="flex items-baseline gap-2 mb-4">
                <span className={`text-3xl sm:text-4xl font-bold transition-all duration-300 ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>{totalCalories}</span>
                <span className={`text-sm transition-all duration-300 ${isDark ? 'text-orange-400/60' : 'text-slate-400'}`}>/ {dailyGoals.calories} kcal</span>
              </div>
              <div className={`w-full rounded-full h-3 transition-colors duration-300 ${isDark ? 'bg-orange-900/50' : 'bg-orange-100'}`}>
                <div
              ...
              <p className={`text-xs font-bold mt-3 uppercase tracking-wider transition-colors duration-300 ${isDark ? 'text-orange-400/80' : 'text-orange-700'}`}>
                {caloriesRemaining > 0 ? `${caloriesRemaining} remaining` : `${Math.abs(caloriesRemaining)} over goal`}
              </p>
            </div>

            {/* Water Card */}
            <div className={`rounded-2xl p-5 sm:p-6 border shadow-sm transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-blue-900/30 to-blue-800/20 border-blue-700/50' : 'bg-white border-blue-100 shadow-blue-100/50'}`}>
              <div className="flex items-center justify-between mb-3">
                <h3 className={`text-sm font-bold uppercase tracking-wide transition-colors duration-300 ${isDark ? 'text-blue-300' : 'text-blue-600'}`}>Water</h3>
                <span className="text-2xl">💧</span>
              </div>
              <div className="flex items-baseline gap-2 mb-4">
                <span className={`text-3xl sm:text-4xl font-bold transition-all duration-300 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>{waterIntake}</span>
                <span className={`text-sm transition-all duration-300 ${isDark ? 'text-blue-300/60' : 'text-slate-400'}`}>/ 8 glasses</span>
              </div>
              <div className={`w-full rounded-full h-3 transition-colors duration-300 ${isDark ? 'bg-blue-900/50' : 'bg-blue-100'}`}>
                <div
              ...
              <button
                onClick={addWater}
                className={`w-full mt-3 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:shadow transition-all ${isDark ? 'bg-blue-600/20 border border-blue-500/30 text-blue-400 hover:bg-blue-600 hover:text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
              >
                + Add Glass
              </button>
            </div>
            ...
            {/* Progress Card */}
            <div className={`rounded-2xl p-5 sm:p-6 border shadow-sm transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-purple-900/30 to-purple-800/20 border-purple-700/50' : 'bg-white border-purple-100 shadow-purple-100/50'}`}>
              <div className="flex items-center justify-between mb-3">
                <h3 className={`text-sm font-bold uppercase tracking-wide transition-colors duration-300 ${isDark ? 'text-purple-300' : 'text-purple-600'}`}>Daily Progress</h3>
                <span className="text-2xl">📊</span>
              </div>
              <div className="flex items-baseline gap-2 mb-4">
                <span className={`text-3xl sm:text-4xl font-bold transition-all duration-300 ${isDark ? 'text-purple-400' : 'text-purple-600'}`}>
                  {((totalCalories / dailyGoals.calories) * 100).toFixed(0)}%
                </span>
                <span className={`text-sm transition-all duration-300 ${isDark ? 'text-purple-300/60' : 'text-slate-400'}`}>achieved</span>
              </div>
              <div className={`w-full rounded-full h-3 transition-colors duration-300 ${isDark ? 'bg-purple-900/50' : 'bg-purple-100'}`}>
                <div
                  className="h-3 bg-gradient-to-r from-purple-500 to-purple-600 rounded-full transition-all duration-500"
                  style={{ width: `${Math.min((totalCalories / dailyGoals.calories) * 100, 100)}%` }}
                />
              </div>
              <p className={`text-xs font-bold mt-3 uppercase tracking-wider transition-colors duration-300 ${isDark ? 'text-purple-400/80' : 'text-purple-700'}`}>
                {totalCalories < dailyGoals.calories ? '📈 On Track' : '✅ Goal Reached! 🎉'}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Macros Breakdown */}
      {activeTrackerTab === 'today' && (
        <div className={`rounded-2xl p-5 sm:p-6 border shadow-sm transition-all duration-300 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
          <h3 className={`text-lg font-bold mb-4 transition-colors duration-300 ${isDark ? 'text-white' : 'text-slate-900'}`}>📊 Macronutrients</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Protein */}
            <div className={`rounded-xl p-4 border transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-red-900/30 to-red-800/20 border-red-700/50 shadow-red-900/10' : 'bg-red-50 border-red-100 shadow-sm shadow-red-100'}`}>
              <div className="flex items-center justify-between mb-2">
                <h4 className={`font-bold transition-colors duration-300 ${isDark ? 'text-red-300' : 'text-red-600'}`}>Protein</h4>
                <span className="text-lg">🥩</span>
              </div>
              <div className="flex items-baseline gap-1 mb-3">
                <span className={`text-2xl font-bold transition-colors duration-300 ${isDark ? 'text-red-400' : 'text-red-600'}`}>{totalProtein.toFixed(0)}</span>
                <span className={`text-sm transition-colors duration-300 ${isDark ? 'text-red-400/60' : 'text-red-500'}`}>/ {dailyGoals.protein}g</span>
              </div>
              <div className={`w-full rounded-full h-2.5 transition-colors duration-300 ${isDark ? 'bg-red-900/50' : 'bg-white'}`}>
                <div
                  className="h-2.5 bg-gradient-to-r from-red-500 to-red-600 rounded-full transition-all duration-500"
            ...
            {/* Carbs */}
            <div className={`rounded-xl p-4 border transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-amber-900/30 to-amber-800/20 border-amber-700/50 shadow-amber-900/10' : 'bg-amber-50 border-amber-100 shadow-sm shadow-amber-100'}`}>
              <div className="flex items-center justify-between mb-2">
                <h4 className={`font-bold transition-colors duration-300 ${isDark ? 'text-amber-300' : 'text-amber-600'}`}>Carbs</h4>
                <span className="text-lg">🍞</span>
              </div>
              <div className="flex items-baseline gap-1 mb-3">
                <span className={`text-2xl font-bold transition-colors duration-300 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>{totalCarbs.toFixed(0)}</span>
                <span className={`text-sm transition-colors duration-300 ${isDark ? 'text-amber-400/60' : 'text-amber-500'}`}>/ {dailyGoals.carbs}g</span>
              </div>
              <div className={`w-full rounded-full h-2.5 transition-colors duration-300 ${isDark ? 'bg-amber-900/50' : 'bg-white'}`}>
                <div
                  className="h-2.5 bg-gradient-to-r from-amber-500 to-amber-600 rounded-full transition-all duration-500"
            ...
            {/* Fats */}
            <div className={`rounded-xl p-4 border transition-all duration-300 ${isDark ? 'bg-gradient-to-br from-yellow-900/30 to-yellow-800/20 border-yellow-700/50 shadow-yellow-900/10' : 'bg-yellow-50 border-yellow-100 shadow-sm shadow-yellow-100'}`}>
              <div className="flex items-center justify-between mb-2">
                <h4 className={`font-bold transition-colors duration-300 ${isDark ? 'text-yellow-300' : 'text-yellow-600'}`}>Fats</h4>
                <span className="text-lg">🥑</span>
              </div>
              <div className="flex items-baseline gap-1 mb-3">
                <span className={`text-2xl font-bold transition-colors duration-300 ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>{totalFats.toFixed(0)}</span>
                <span className={`text-sm transition-colors duration-300 ${isDark ? 'text-yellow-400/60' : 'text-yellow-500'}`}>/ {dailyGoals.fats}g</span>
              </div>
              <div className={`w-full rounded-full h-2.5 transition-colors duration-300 ${isDark ? 'bg-yellow-900/50' : 'bg-white'}`}>
                <div
                  className="h-2.5 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-full transition-all duration-500"
      ...
      {activeTrackerTab === 'today' && (
        <div className={`rounded-2xl p-5 sm:p-6 border transition-all duration-300 shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          <h3 className={`text-lg font-bold mb-4 transition-colors duration-300 ${isDark ? 'text-white' : 'text-slate-900'}`}>➕ Add Food</h3>

          {/* Tabs */}
          <div className="flex gap-2 mb-5">
            <button
              onClick={() => setActiveTab('manual')}
              className={`px-4 py-2.5 rounded-xl font-semibold text-sm transition-all ${activeTab === 'manual'
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl font-semibold text-sm transition-all ${activeTab === 'scan'
                ? 'bg-green-500 text-white shadow-md'
                : 'bg-gray-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-gray-600'
                }`}
            >
          ...
          {activeTab === 'manual' && (
            <div className="space-y-4">
              ...
              <div>
                <label className={`block text-sm font-semibold mb-2 transition-colors duration-300 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Food Name</label>
                <input
                  type="text"
                  value={foodName}
                  onChange={(e) => setFoodName(e.target.value)}
                  placeholder="e.g., Chicken Breast, Brown Rice"
                  className={`w-full px-4 py-3 border rounded-xl outline-none transition-all text-base ${isDark ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500 focus:border-green-500 focus:ring-2 focus:ring-green-900/20' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-green-500 focus:ring-2 focus:ring-green-100'
                    }`}
              ...
              <div>
                <label className={`block text-sm font-semibold mb-2 transition-colors duration-300 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Notes (Optional)</label>
                <input
                  type="text"
                  value={mealNotes}
      ...
      {activeTrackerTab === 'today' && (
        <div className={`rounded-2xl p-5 sm:p-6 border transition-all duration-300 shadow-sm ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          <h3 className={`text-lg font-bold mb-4 flex items-center justify-between transition-colors duration-300 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            <span>📋 Today's Meals</span>
            <span className={`text-sm font-medium px-3 py-1 rounded-full ${isDark ? 'text-gray-400 bg-gray-700' : 'text-gray-500 bg-gray-100'}`}>
          ...
          {dailyLogs.length === 0 ? (
            <div className="text-center py-12 px-4 transition-all duration-300">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 transition-colors duration-300 ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
                <span className="text-4xl">🍽️</span>
              </div>
              <p className={`text-lg font-semibold mb-2 transition-colors duration-300 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>No meals logged yet</p>
              <p className={`text-sm transition-colors duration-300 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Start by adding a food or scanning a meal photo</p>
            </div>
          ) : (
            <div className="space-y-6">
