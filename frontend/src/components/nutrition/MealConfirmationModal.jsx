...
export default function MealConfirmationModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          ...
          {avgConfidence < 0.7 && avgConfidence > 0 && (
            <div className={`border rounded-xl p-4 flex gap-3 ${isDark ? 'bg-yellow-900/20 border-yellow-700/50' : 'bg-yellow-50 border-yellow-200'}`}>
              <AlertCircle className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isDark ? 'text-yellow-500' : 'text-yellow-600'}`} />
              <div>
                <p className={`font-semibold ${isDark ? 'text-yellow-400' : 'text-yellow-900'}`}>Low confidence detected</p>
                <p className={`text-sm mt-1 ${isDark ? 'text-yellow-500/80' : 'text-yellow-800'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {['breakfast', 'lunch', 'dinner', 'snack'].map((type) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-all capitalize ${mealType === type
          ...
          {/* Food Items */}
          <div>
            <h3 className={`font-semibold mb-3 ${isDark ? 'text-white' : 'text-gray-900'}`}>Detected Foods</h3>
            {items.length === 0 ? (
              <div className={`text-center py-8 rounded-xl border ${isDark ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                <AlertCircle className={`w-12 h-12 mx-auto mb-3 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                <p className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>No foods detected</p>
              ...
              <div className="space-y-3">
                {items.map((item, index) => (
                  <div
                    key={index}
                    className={`border rounded-xl p-4 transition-colors ${isDark ? 'bg-gray-700/50 border-gray-600 hover:bg-gray-700' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}
                  >
          ...
          {/* Total Summary */}
          {items.length > 0 && (
            <div className={`rounded-xl p-4 border ${isDark ? 'bg-gray-800/50 border-gray-600' : 'bg-gray-50 border-gray-200 shadow-sm'}`}>
              <h4 className={`font-semibold mb-3 ${isDark ? 'text-white' : 'text-gray-900'}`}>Meal Total</h4>
              <div className="grid grid-cols-4 gap-2 text-center">
                <div>
                  <p className={`text-2xl font-bold ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>
                    {totals.calories}
                  </p>
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Calories</p>
                </div>
                <div>
                  <p className={`text-2xl font-bold ${isDark ? 'text-red-400' : 'text-red-600'}`}>
                    {totals.protein.toFixed(1)}g
                  </p>
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Protein</p>
                </div>
                <div>
                  <p className={`text-2xl font-bold ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                    {totals.carbs.toFixed(1)}g
                  </p>
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Carbs</p>
                </div>
                <div>
                  <p className={`text-2xl font-bold ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`}>
                    {totals.fats.toFixed(1)}g
                  </p>
                  <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Fats</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className={`flex gap-3 pt-4 border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
            <button
              onClick={onCancel}
              disabled={saving}
              className={`flex-1 px-4 py-3 rounded-lg font-semibold disabled:opacity-50 transition-colors ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}
            >
              Cancel
            </button>
            <button
